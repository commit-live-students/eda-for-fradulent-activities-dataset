...
print test.head(5)
print len(user['country'].value_counts())

def join_dataframe(df1, df2):
    ret_df = pd.merge(df1,df2,on='user_id')
    return ret_df
...
def get_categorical_variables(df):
    ...
    for ele in col_names:
        ...
        if len(df[ele].value_counts().index)<18:
...
def get_numerical_variables(df):
    ...
    ls = []
    for ele in col_names:
        if (df[ele].dtype == 'int64' or df[ele].dtype == 'float64') & (ele not in get_categorical_variables(df)):
            ls.append(ele)
    df2 = pd.DataFrame(df,columns=ls)
    return df2
...
def get_numerical_variables_percentile(df):
    ...
    ls2 = get_numerical_variables(df).columns.tolist()
